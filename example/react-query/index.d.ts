declare module 'react-query' {
   ...
   */
  export type QueryKeyVariables<T extends QueryKey> = T extends [
    string,
    infer U,
    ...any[]
  ]
   ...
   */
  export type QueryKeyVariablesArgs<T extends QueryKey> = T extends any[]
    ? TupleTail<T>
    : [];
   ...
   */
  export type TupleTail<A extends any[]> = ((...args: A) => any) extends (
    h: any,
    ...t: infer T
  ) => any
    ? T
  ...
  export function useQuery<TResult, TQueryKey extends QueryKey>(
    ...
    queryFn: (
      key: string,
      ...args: QueryKeyVariablesArgs<TQueryKey>
    ) => Promise<TResult>,
  ...
  export interface InfiniteQueryOptions<TResult, TVariables>
    extends QueryOptions<TResult> {
    getFetchMore?:
      | ((lastGroup: TResult, allGroups: TResult[]) => TVariables)
      | boolean;
  }
  ...
  export interface QueryLoadingState<TVariables>
    extends QueryBaseState<TVariables> {
    status: 'loading';
    data: undefined;
    error: null;
  }

  export interface QueryErrorState<TVariables>
    extends QueryBaseState<TVariables> {
    status: 'error';
    data: undefined;
    error: Error;
  ...
  export interface QuerySuccessState<TResult, TVariables>
    extends QueryBaseState<TVariables> {
    status: 'success';
    data: TResult;
    error: null;
  ...
  export interface PaginatedQueryLoadingState<TVariables>
    extends QueryBaseState<TVariables> {
    status: 'loading';
  ...
  export interface PaginatedQueryErrorState<TVariables>
    extends QueryBaseState<TVariables> {
    status: 'error';
    resolvedData: undefined;
    latestData: undefined;
  ...
  export interface PaginatedQuerySuccessState<TResult, TVariables>
    extends QueryBaseState<TVariables> {
    status: 'success';
  ...
  export interface InfiniteQueryLoadingState<TVariables>
    extends InfiniteQueryBaseState<TVariables> {
    status: 'loading';
    data: [];
    error: null;
  }

  export interface InfiniteQueryErrorState<TVariables>
    extends InfiniteQueryBaseState<TVariables> {
    status: 'error';
    data: [];
    error: Error;
  ...
  export interface InfiniteQuerySuccessState<TResult, TVariables>
    extends InfiniteQueryBaseState<TVariables> {
    status: 'success';
    data: TResult[];
    error: null;
  ...
  export namespace queryCache {
    ...
    ): (unsubscribe: (...args: any[]) => void) => void;
